...
export const Header = () => {
    ...
    //Chuyển đổi API getSubjects sang async/await try-catch
    useEffect(() => {
        const fetchSubjectsHeader = async () => {
            try {
                const res = await getSubjects();
                setSubjects(res.data);
            } catch (err) {
                console.error("Lỗi khi tải danh sách môn học trên Header:", err);
            }
        };
                                ...
                                <div className="document-dropdown">
                                    {subjects.map((item) => (
                                        <div
                                            ...
                                            onClick={() => navigate(`/document/${item.subject_id}`)}
                                        >
                                            {item.subject_name}
                                        </div>
                                    ))}
